...
import hashlib
from ..models.experiment import VariantConfig, ExperimentType
...
class BucketingService:
    ...
    def assign_variant(
        ...
        if total_allocation != 100:
            ...
            for variant in variants:
                variant_buckets = (variant.traffic_percentage * scale_factor / 100) * self.bucket_count
    ...
    def get_assignment_ranges(self, variants: list[VariantConfig]) -> Dict[str, range]:
        ...
        cumulative = 0
        for variant in variants:
            variant_buckets = int((variant.traffic_percentage * scale_factor / 100) * self.bucket_count)
