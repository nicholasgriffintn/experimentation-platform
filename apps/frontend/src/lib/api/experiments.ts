...
export class ExperimentsAPI extends BaseAPI {
  ...
  async assignVariant(
    ...
    return this.post<VariantAssignment>(`/${experimentId}/assign`, userContext);
  }
  ...
  async recordExposure(
    ...
    await this.post(`/${experimentId}/exposure`, { ...userContext, metadata });
  }
  ...
  async getResults(
    ...
    const query = metrics?.length ? `?metrics=${metrics.join(",")}` : "";
